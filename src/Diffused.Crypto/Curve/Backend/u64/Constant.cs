...
namespace Diffused.Crypto.Curve
{
    public static partial class Constant
    {
        ...
        public static FieldElement EDWARDS_D = new FieldElement(new ulong[]
        {
            929955233495203, 466365720129213, 1662059464998953, 2033849074728123, 1442794654840575
        });

        /// Edwards `2*d` value, equal to `2*(-121665/121666) mod p`.
        public static FieldElement EDWARDS_D2 = new FieldElement(new ulong[]
        {
            1859910466990425, 932731440258426, 1072319116312658, 1815898335770999, 633789495995903
        });

        /// `= sqrt(a*d - 1)`, where `a = -1 (mod p)`, `d` are the Edwards curve parameters.
        public static FieldElement SQRT_AD_MINUS_ONE = new FieldElement(new ulong[]
        {
        ...
        public static FieldElement INVSQRT_A_MINUS_D = new FieldElement(new ulong[]
        {
            278908739862762, 821645201101625, 8113234426968, 1777959178193151, 2118520810568447
        });

        /// Precomputed value of one of the square roots of -1 (mod p)
        public static FieldElement SQRT_M1 = new FieldElement(new ulong[]
        {
            1718705420411056, 234908883556509, 2233514472574048, 2117202627021982, 765476049583133
        });

        /// `APLUS2_OVER_FOUR` is (A+2)/4. (This is used internally within the Montgomery ladder.)
        public static FieldElement APLUS2_OVER_FOUR = new FieldElement(new ulong[]
        {
            121666, 0, 0, 0, 0
        });

        /// `L` is the order of base point, i.e. 2^252 + 27742317777372353535851937790883648493
        public static UnpackedScalar L = new UnpackedScalar(new ulong[]
        {
            0x0002631a5cf5d3ed, 0x000dea2f79cd6581, 0x000000000014def9, 0x0000000000000000, 0x0000100000000000
        });

        /// `L` * `LFACTOR` = -1 (mod 2^52)
        public static ulong LFACTOR = 0x51da312547e1b;

        /// `R` = R % L where R = 2^260
        public static UnpackedScalar R = new UnpackedScalar(new ulong[]
        {
            0x000f48bd6721e6ed, 0x0003bab5ac67e45a, 0x000fffffeb35e51b, 0x000fffffffffffff, 0x00000fffffffffff
        });

        /// `RR` = (R^2) % L where R = 2^260
        public static UnpackedScalar RR = new UnpackedScalar(new ulong[]
        {
            0x0009d265e952d13b, 0x000d63c715bea69f, 0x0005be65cb687604, 0x0003dceec73d217f, 0x000009411b7c309a
        });
